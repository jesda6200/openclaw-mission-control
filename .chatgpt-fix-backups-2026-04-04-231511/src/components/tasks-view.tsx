...
export function TasksView() {
  ...
  const runningDispatches = tasks.filter((t) => t.dispatchStatus === "running").length;
  const completionPct =
      ...
      <div className="shrink-0 space-y-3 px-4 md:px-6 pt-5 pb-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <span className={cn("h-1.5 w-1.5 rounded-full", liveConnected ? "bg-emerald-400" : "bg-amber-400 animate-pulse")} />
              Live updates {liveConnected ? "on" : "reconnecting"}
            </span>
            {lastLiveUpdateAt && (
              <span className="text-xs text-muted-foreground/50">
                updated {new Date(lastLiveUpdateAt).toLocaleTimeString()}
              </span>
            )}
            {saveStatus && (
              <span
