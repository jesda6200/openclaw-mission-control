...
function buildGraph(
  ...
  const GATEWAY_X = -520;
  const GATEWAY_Y = 110;
  const CHANNEL_COLUMN_X = -760;
  const CHANNEL_START_Y = -210;
  const CHANNEL_SPACING_Y = 92;
  const MAIN_X = 120;
  const MAIN_Y = -220;
  const GROUP_START_X = 120;
  const GROUP_SPACING_X = 330;
  const GROUP_Y = 110;
  const AGENT_Y_BASE = 260;
  const AGENT_Y_STEP = 108;
  const WORKSPACE_ROW_Y = 720;
  const WORKSPACE_SPACING_X = 280;
  const RUNTIME_SUBAGENT_OFFSET_Y = -100;
  const RUNTIME_SUBAGENT_SPACING_X = 200;
  ...
  const configuredById = new Map(MISSION_CONTROL_GROUPS.map((group) => [group.id, group] as const));
  const groups = MISSION_CONTROL_GROUP_ORDER.map((groupId) => {
    const group = configuredById.get(groupId)!;
    return {
      ...group,
  ...
  const canonicalIds = new Set([
    MISSION_CONTROL_MAIN_AGENT_ID,
    ...MISSION_CONTROL_GROUPS.flatMap((group) => group.agents),
  ]);
  const outOfScopeAgents = agents.filter((agent) => !canonicalIds.has(agent.id));
  if (outOfScopeAgents.length > 0) {
    groups.push({
      id: "hors-perimetre",
      label: "HORS PERIMETRE",
      agents: outOfScopeAgents.map((agent) => agent.id),
      resolved: outOfScopeAgents,
  ...
  if (mainAgent) {
    ...
    fallbackPositions.set(nodeId, { x: MAIN_X, y: MAIN_Y });
    nodes.push({
      id: nodeId,
      type: "agent",
      position: getPosition(nodeId, MAIN_X, MAIN_Y),
      data: {
        agent: mainAgent,
  ...
  groups.forEach((group, groupIndex) => {
    ...
    const groupColumns = Math.max(1, Math.ceil(group.resolved.length / 3));
    const groupHeight = 180 + Math.ceil(group.resolved.length / groupColumns) * 102;
    ...
    group.resolved.forEach((agent, agentIndex) => {
      ...
      const col = agentIndex % groupColumns;
      const row = Math.floor(agentIndex / groupColumns);
      const fallbackX = groupX + (col - (groupColumns - 1) / 2) * 240;
      const fallbackY = AGENT_Y_BASE + row * 112;
  ...
  const chFallbackSpacing = CHANNEL_SPACING_Y;
  channels.forEach(([ch, accountIds], i) => {
    const nodeId = `ch-${ch}`;
    const fallbackY = CHANNEL_START_Y + i * chFallbackSpacing;
...
function FlowViewInner({
  ...
  return (
    <ReactFlow
      ...
      onInit={() => {
        setTimeout(() => fitView({ padding: 0.18, maxZoom: 0.9 }), 0);
        setTimeout(() => fitView({ padding: 0.18, maxZoom: 0.9, duration: 200 }), 300);
      }}
