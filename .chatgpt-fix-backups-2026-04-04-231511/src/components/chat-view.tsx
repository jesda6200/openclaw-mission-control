...
function taskProgressPercent(task: TaskProgress): number {
  if (task.dispatchStatus === "completed" || task.dispatchStatus === "failed") return 100;
  if (task.dispatchStatus === "running") {
    if (!task.dispatchedAt) return 20;
    const elapsed = Date.now() - task.dispatchedAt;
    return Math.min(95, Math.max(10, 15 + elapsed / 15000));
  }
  return 0;
}
...
function ChatPanel({
  ...
  const [tasksLiveConnected, setTasksLiveConnected] = useState(false);
  ...
  useEffect(() => {
    ...
    es.onopen = () => setTasksLiveConnected(true);
    es.onmessage = (e) => {
      try {
        const payload = JSON.parse(e.data);
        if (payload?.type === "kanban-updated") loadTasks();
      } catch {
        /* ignore */
      }
    };
    es.onerror = () => setTasksLiveConnected(false);

    return () => {
      es.close();
      taskStreamRef.current = null;
      setTasksLiveConnected(false);
    };
  }, []);
  ...
  return (
    ...
    >
      {/* ── Tasks progress banner ────────────────── */}
      <div className="shrink-0 border-b border-foreground/5 bg-card/40 px-4 py-2 md:px-6">
        <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
          <div className="flex items-center gap-2">
            ...
            >
              <span className={cn("h-1.5 w-1.5 rounded-full", tasksLiveConnected ? "bg-emerald-400" : "bg-amber-400 animate-pulse")} />
              Tasks live {tasksLiveConnected ? "on" : "reconnecting"}
            </span>
            <span className="text-muted-foreground/70">
              {tasks.filter((t) => t.dispatchStatus === "running").length} running
            </span>
          </div>
          <div className="flex-1 min-w-40 max-w-xs">
            <div className="h-1.5 overflow-hidden rounded-full bg-foreground/5">
              <div
                ...
                style={{ width: `${Math.min(100, (tasks.filter((t) => t.dispatchStatus === "completed").length / Math.max(tasks.length, 1)) * 100)}%` }}
              />
            </div>
          </div>
        </div>
        <div className="mt-2 space-y-1.5">
          {tasks.filter((t) => t.dispatchStatus === "running").length > 0 ? (
            tasks.filter((t) => t.dispatchStatus === "running").slice(0, 3).map((task) => {
              const pct = taskProgressPercent(task);
              return (
                <div key={task.id} className="rounded-lg border border-amber-500/15 bg-amber-500/5 px-2.5 py-2">
                  <div className="flex items-center justify-between gap-2 text-[11px]">
                    <span className="inline-flex min-w-0 items-center gap-1 text-amber-400">
                      <Loader2 className="h-3 w-3 shrink-0 animate-spin" />
                      <span className="truncate">#{task.id} {task.title}</span>
                    </span>
                    <span className="shrink-0 font-medium text-amber-300">{Math.round(pct)}%</span>
                  </div>
            ...
            <div className="rounded-lg border border-dashed border-foreground/10 bg-foreground/5 px-2.5 py-2 text-[11px] text-muted-foreground/60">
              No tasks running
            </div>
          )}
        </div>
